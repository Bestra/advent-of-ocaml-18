...
let parse_entry e = int_of_char e

let read_file () =
  In_channel.read_lines "./inputs/1.txt" |> List.map ~f:int_of_string

let part1 () =
  let lines = read_file () in
  List.fold ~init:0 ~f:(fun acc x -> acc + x) lines

type search_state = {seen: Base.Set.M(Int).t; previous: int}

let rec reachedTwice (a_list : int sexp_list) count list_length
    (state : search_state) =
